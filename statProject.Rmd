...
The TRUE and FALSE labels are balanced in both the training and test sets.

We build a first model considering all the variables:
...
COMPARING WITH A DUMMY DATASET
```{r}
th <- coords(roc.out, "best")
probabilities <- logit.out1 %>% predict(test, type = "response")
...
Based on the P-values obtained from the previous model, we decide to remove the holiday feature, as it's the highest.


```{r}
logit.out2 <- glm(data$high_RRP ~ data$demand + data$mean_temperature + data$solar_exposure + data$rainfall+ data$school_day + data$season, family = binomial)

summary(logit.out2)
```
```{r}
logistic.prob <- predict(logit.out2, type="response")
roc.out <- roc(data$high_RRP, logistic.prob, levels=c(FALSE, TRUE))

plot(roc.out, print.auc=TRUE, legacy.axes=TRUE, xlab="False positive rate", ylab="True positive rate")
print(coords(roc.out, "best"))
```

The AUC worsened with respect to the first model. Considering the P-values obtained through this new model, we decide to remove the solar exposure feature next.

```{r}
logit.out3 <- glm(data$high_RRP ~ data$demand + data$mean_temperature + data$rainfall+ data$school_day + data$season, family = binomial)
...
```{r}
logistic.prob <- predict(logit.out3, type="response")
roc.out <- roc(data$high_RRP, logistic.prob, levels=c(FALSE, TRUE))
...
AUC improved a little with respect to the second model, but it is still worse than the first one. We proceed to remove the rainfall variable.

```{r}
logit.out4 <- glm(data$high_RRP ~ data$demand + data$mean_temperature + data$school_day + data$season, family = binomial)
...
print(coords(roc.out, "best"))
```
Again, AUC worsened with respect to the previous model.

Now we build a model without mean temperature:

```{r}
logit.out5 <- glm(data$high_RRP ~ data$demand + data$school_day + data$season, family = binomial)

summary(logit.out5)
```
```{r}
logistic.prob <- predict(logit.out5, type="response")
roc.out <- roc(data$high_RRP, logistic.prob, levels=c(FALSE, TRUE))
...
```{r}
logit.out6 <- glm(data$high_RRP ~ data$demand + data$school_day , family = binomial)

summary(logit.out6)
```

```{r}
logistic.prob <- predict(logit.out6, type="response")
roc.out <- roc(data$high_RRP, logistic.prob, levels=c(FALSE, TRUE))
...
Again, AUC worsened with respect to the previous model. Based on these results we should keep the logit.out5 model. 

Now, we try to use the months as a feature instead of the seasons, and consider all the other initial variables:
...
CM <- table(trivial.pred, test$high_RRP)

CM
```

ERROR RATE FROM TEST AND TRIVIAL PRECITOR
```{r}

print((145+198)/(145 + 121+ 159+ 198))
