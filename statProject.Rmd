...
figure1
```
The above graphs show that while demand does not fluctuate much over the years, RRP does shift considerably during and throughout the years. There there was a notable peak in RRP around 2017 that lasted until almost 2020, when it went down again.
...
ggplot(data, aes(x=date)) +
  ...
  geom_line( aes(y=demand/coeff),color = 'blue') + 
  scale_y_continuous(name = "Mean Temperature",sec.axis = sec_axis(~.*coeff, name="Demand")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.line.y.right = element_line(color = "blue"),
        axis.line.y.left = element_line(color = "red"))
```

From the above graph, it is very clear that when mean temperature peaks, demand goes down and viceversa. This suggests that more electricity is used when the weather is colder.
...
The same can be concluded from the above graphs.As temperature goes down, demand rises.
...
Again, we conclude that the variances of the two populations are not equal.
...
The BIC (Bayesian Information Criterion) for this simple model is the following:
...
Next, we build a linear model for demand with only the mean temperature variable.
...
Next, we build a model considering only the categorical variables.
...
We start removing variables to find the model which gives the highest Adjusted R squared. We start with holiday:
...
We move on to remove season and holiday:
...
Then we remove season and school day:
...
And finally we remove holiday and school day:
